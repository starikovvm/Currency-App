...
- (NSArray<SCAConversionDirection *> *)possibleConversionDirections {
    ...
    dispatch_once(&onceToken, ^{
        conversionDirections = @[[SCAConversionDirection conversionDirectionFrom:kUSDSymbol to:kRUBSymbol], [SCAConversionDirection conversionDirectionFrom:kUSDSymbol to:kEURSymbol], [SCAConversionDirection conversionDirectionFrom:kEURSymbol to:kRUBSymbol], [SCAConversionDirection conversionDirectionFrom:kEURSymbol to:kUSDSymbol], [SCAConversionDirection conversionDirectionFrom:kRUBSymbol to:kUSDSymbol], [SCAConversionDirection conversionDirectionFrom:kRUBSymbol to:kEURSymbol]];
    });
...
- (void)initialize {
    ...
    RACSignal *fetchTodayPriceSignal = [[[self fetchJSONFromURL:[NSURL URLWithString:kTodayURLString]] doNext:^(id response) {
        ...
        if ([response isKindOfClass:[NSDictionary class]]) {
            SCAConversionRate *todayConversionRate = [[SCAConversionRate alloc] initWithDictionary:response];
            if (todayConversionRate.baseCurrency && todayConversionRate.currencyRates) {
                self.todayConversionRate = todayConversionRate;
            }
        }
    }] deliverOnMainThread];
        ...
        return [fetchTodayPriceSignal subscribeNext:^(id x) {
            ...
            [[self fetchYesterdayRatesWithToday:x[@"date"]] subscribeNext:^(id yesterdayResponse) {
                if ([yesterdayResponse isKindOfClass:[NSDictionary class]]) {
                    SCAConversionRate *yesterdayConversionRate = [[SCAConversionRate alloc] initWithDictionary:yesterdayResponse];
                    if (yesterdayConversionRate.baseCurrency && yesterdayConversionRate.currencyRates) {
                        self.yesterdayConversionRate = yesterdayConversionRate;
...
- (void)setCurrentConversionDirectionIndex:(NSUInteger)currentConversionDirectionIndex {
...
- (RACSignal *)fetchYesterdayRatesWithToday:(NSString *)today {
    RACSignal *signal = [RACSignal createSignal:^RACDisposable *(id <RACSubscriber> subscriber) {
        ...
        return [[self fetchJSONFromURL:[NSURL URLWithString:urlString]]
