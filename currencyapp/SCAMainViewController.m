...
- (void)viewDidLoad {
    ...
    RAC(self.updateDateLabel, text, [NSLocalizedString(@"DATA_UPDATING", @"data updating status message") uppercaseString]) =
            [[RACObserve(self.viewModel, dataUpdatedDate) map:^NSString *(NSDate *date) {
    ...
    [self.viewModel.currentConversionDirectionSignal subscribeNext:^(NSNumber *index) {
        ...
        self.currentConversionDirectionLabel.attributedText = [[NSAttributedString alloc] initWithString:self.viewModel.possibleConversionDirectionsSequence.array[[index unsignedIntegerValue]] ?: @"" attributes:[self currentConversionDirectionLabelTextAttributes]];
    }];
    ...
    [self.viewModel.todayPriceSignal subscribeNext:^(NSDecimalNumber *todayPrice) {
        ...
        @strongify(self);
        self.updateDateLabel.text = NSLocalizedString(@"ERROR_TODAY_PRICE", @"Error while fetching today price");
    }];
    ...
    [self.viewModel.yesterdayPriceSignal subscribeNext:^(NSDecimalNumber *yesterdayPrice) {
        ...
        self.yesterdayDifferenceLabel.attributedText = [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:formatString, NSLocalizedString([self.viewModel currentToSymbolString], nil), [self.percentNumberFormatter stringFromNumber:yesterdayPrice]] attributes:[self yesterdayDifferenceLabelTextAttributes]];
    }                                            error:^(NSError *error) {
        @strongify(self);
        self.updateDateLabel.text = NSLocalizedString(@"ERROR_YESTERDAY_PRICE", @"Error while fetching yesterday price");
    }];

    [self.viewModel.showCurrencySelectionSignal subscribeNext:^(NSNumber *showSelection) {
        @strongify(self);
        [self setDateSelectionHidden:! [showSelection boolValue] animated:self.verticalConstraints != nil];
    }];
...
- (NSNumberFormatter *)percentNumberFormatter {
    if (! _percentNumberFormatter) {
        ...
        _percentNumberFormatter.minimumFractionDigits = 0;
        _percentNumberFormatter.maximumFractionDigits = 2;
        _percentNumberFormatter.minimumIntegerDigits = 1;
        _percentNumberFormatter.roundingMode = NSNumberFormatterRoundHalfEven;
        _percentNumberFormatter.negativePrefix = @"";
        _percentNumberFormatter.decimalSeparator = @",";
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    self.viewModel.currentConversionDirectionIndex = (NSUInteger)indexPath.row;

}

- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath {
    SCACurrencySelectTableViewCell *currencySelectTableViewCell = (SCACurrencySelectTableViewCell *)cell;
    currencySelectTableViewCell.currencyLabel.text = self.viewModel.possibleConversionDirectionsSequence.array[(NSUInteger)indexPath.row];
}
